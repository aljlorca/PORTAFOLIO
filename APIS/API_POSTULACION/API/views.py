...
def agregar_postulacion(descripcion,estado,id_venta_externa,id_empresa):
    django_cursor = connection.cursor()
    cursor = django_cursor.connection.cursor()
    salida = cursor.var(cx_Oracle.NUMBER)
    estado_fila = '1'
    cursor.callproc('POSTULACION_AGREGAR',[descripcion,estado,id_venta_externa,id_empresa,estado_fila,salida])
    return salida

def modificar_postulacion(id_postulacion,descripcion,estado,id_venta_externa,id_empresa):
    django_cursor = connection.cursor()
    cursor = django_cursor.connection.cursor()
    salida = cursor.var(cx_Oracle.NUMBER)
    cursor.callproc('POSTULACION_MODIFICAR',[id_postulacion,descripcion,estado,id_venta_externa,id_empresa,salida])
...
class PostulacionView(View):
    ...
    def post(self, request):
        ...
        agregar_postulacion(descripcion=jd['descripcion'],estado=jd['estado'],id_venta_externa=jd['id_venta_externa'],id_empresa=jd['id_empresa'])
    ...
    def put(self, request,id_postulacion):
        ...
        postulaciones = list(Postulacion.objects.filter(id_postulacion=id_postulacion).values())
        if len(postulaciones) > 0:
            modificar_postulacion(id_postulacion=jd['id_postulacion'],descripcion=jd['descripcion'],estado=jd['estado'],id_venta_externa=jd['id_venta_externa'],id_empresa=jd['id_empresa'])
            datos={'message':"Success"}
        else:
